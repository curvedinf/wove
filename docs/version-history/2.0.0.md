...
- Expands `merge(...)` so sync tasks can call it directly, async tasks can await it, and merged callables can use the same execution options as `@w.do(...)`, including remote environments.
