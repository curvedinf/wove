...
async def process_special_user(user_data: dict):
    ...
    print(f"Processing special privileges for user {user_data['id']} ({user_data['name']})...")
...
def generate_standard_user_report(user_data: dict):
    ...
    print(f"Generating standard report for user {user_data['id']} ({user_data['name']})...")
...
async def run_dynamic_workflow_example():
    ...
    assert w.result['process_user_two'] == "Generated report for user: Mrs. Dennis Schulist"
