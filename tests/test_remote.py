...
from wove import remote_worker
from wove.integrations import worker
from wove.remote import (
    RemoteCallbackServer,
    build_remote_payload,
    deserialize_frame,
    payload_from_b64,
    payload_to_b64,
    run_remote_payload,
    run_remote_payload_async,
    serialize_frame,
)
...
async def test_callback_server_idempotent_start_public_url_and_not_started_paths():
    server = RemoteCallbackServer(public_url="http://public/wove/events/token/", token="token")
...
async def test_callback_server_rejects_wrong_path_and_bad_json():
    server = RemoteCallbackServer(token="token")
    await server.start()
...
@pytest.mark.asyncio
async def test_run_remote_payload_posts_error_and_cancel_frames():
    ...
    result = await run_remote_payload_async(make_payload(server.callback_url, boom, run_id="err"))
    ...
    result = await run_remote_payload_async(make_payload(server.callback_url, cancelled, run_id="cancel"))
...
@pytest.mark.asyncio
async def test_sync_worker_entrypoints_and_remote_worker_main(monkeypatch):
    server = RemoteCallbackServer()
    await server.start()
    assert server.callback_url is not None

    payload = make_payload(server.callback_url, lambda value: value * 3, {"value": 4})
    assert await asyncio.to_thread(run_remote_payload, payload) == 12
    assert (await server.recv())["type"] == "task_started"
    ...
    monkeypatch.setattr(sys, "argv", ["wove.remote_worker", payload_to_b64(payload)])
    assert await asyncio.to_thread(remote_worker.main) == 0
    assert (await server.recv())["type"] == "task_started"
    assert (await server.recv())["result"] == "main"

    monkeypatch.setattr(sys, "argv", ["wove.remote_worker"])
    monkeypatch.delenv("WOVE_REMOTE_PAYLOAD", raising=False)
    with pytest.raises(SystemExit, match="Missing remote payload"):
        remote_worker.main()

    with pytest.raises(RuntimeError, match="already inside an event loop"):
        run_remote_payload(payload)
