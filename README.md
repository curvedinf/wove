...
Wove is designed to be added inline in your existing functions. Since it is not required to be in an `async` block, it is useful for retrofiting into any IO-bound parallelizable process. For instance in a Django view, you can have all of your QuerySets run at the same time.
```python
# views.py
...
print(w.result.final)
```
You can override or add any tasks inline in your `with` block. An overridden task inherits its parent's `do` parameters if not specified, with the exception of mappings. If you would like the same mappings for a task, they must be restated.
```python
# In views.py
from wove import weave
from .reports import StandardReport
